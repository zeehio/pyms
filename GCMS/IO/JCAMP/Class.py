"""
Classes for I/O of data in JCAMP-DX format
"""
...
class JCAMP_reader(object):
    ...
    def __init__(self, file_name):
        ...
        """
        @summary: Initialize the reader for JCAMP DX files

        @param file_name: The name of the JCAMP DX file
        ...
        if not is_str(self.__file_name):
            error("'file_name' not a string")

        print " -> Reading JCAMP file '%s'" % (self.__file_name)
        lines_list = open(self.__file_name,'r')
        data = []
        page_idx = 0
        xydata_idx = 0
        time_list = []
        ...
        self.__gcms_data = GCMS_data(time_list, scan_list)

    def get_gcms_data(self):

        """
        @summary: Return the GC-MS data

        @return: return the GC-MS data
        @rtype: pyms.GCMS.Class.GCMS_data
